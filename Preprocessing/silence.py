...
def remove_silence(fs, signal, frame_duration = 0.02, frame_shift = 0.01, perc = 0.15):
...
datafolder = '/Users/1/Desktop/dataset/'
folders = os.listdir(datafolder)[:1]

for folder in folders:
    vadfolder = datafolder + '/' + folder + '/' + 'vad2' #путь до папки vad
    os.makedirs(vadfolder) #создаем директории

    files_in_folder = glob.glob(datafolder + folder + '/wav/' + '*.wav') #список wav файлов в папке
    
    for file in files_in_folder:
        track_name = file.split(folder + '/wav' + '\\')[1]
        vad_wav_file = vadfolder + '/' + track_name  #путь до vad файлов 
        vad(file, vad_wav_file)